...
<div class="container">
  ...
  <div class="card">
    ...
    <div style="margin-top:18px; display:flex; gap:12px;">
      ...
      <a href="/download-report?masked=1" style="text-decoration:none;">
        <button type="button" style="background:#3a4a75;">🔒 Download Masked Report (Privacy Mode)</button>
      </a>
    </div>
  </div>
...
def download_report():
    ...
    masked = request.args.get("masked") == "1"
    out_path = os.path.join(REPORT_FOLDER, f"forensic_report{'_masked' if masked else ''}.pdf")
    case_id, signature_block = generate_pdf_report(report, out_path, masked=masked)
    return send_file(out_path, as_attachment=True, download_name=f"{case_id}{'_MASKED' if masked else ''}.pdf")


@app.route("/verify-signature", methods=["POST"])
def verify_signature_endpoint():
    """Lets anyone (a judge, an investigator) re-verify that a report's
    findings haven't been tampered with, using the stored signature."""
    report = app.config.get("LAST_REPORT")
    if not report:
        return {"verified": False, "reason": "no report loaded"}, 400
    signature_block = crypto_signing.sign_report(report)  # re-derive for demo purposes
    is_valid = crypto_signing.verify_report(report, signature_block)
    return {"verified": is_valid, "hash": signature_block["sha256_hash"]}
