...
<div class="container">
  ...
  <div class="card">
    ...
    <table>
      ...
      <tr><td class="label">This Email's Score</td><td class="{{ 'fail' if report.threat_assessment.ml_classifier.probability and report.threat_assessment.ml_classifier.probability >= 0.5 else 'pass' }}">
        {{ "%.1f"|format(report.threat_assessment.ml_classifier.probability * 100) }}% phishing/spam confidence \u2014 classified as <b>{{ report.threat_assessment.ml_classifier.label|upper }}</b>
      </td></tr>
    </table>
  </div>
