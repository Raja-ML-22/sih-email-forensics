"""
Forensic Report Generator (PDF) — SIH 2026, PS ID 26106, Team Tech Titans
"""
...
from privacy import apply_privacy_mask, RETENTION_POLICY_TEXT


def _esc(text):
    """Escapes XML special characters so raw email content (which often
    contains <angle-bracket addresses> and & characters) renders as literal
    text instead of being silently swallowed as markup by ReportLab."""
...
def _add_kv_table(elements, rows):
    ...
    data = [[Paragraph(_esc(k), label_style), Paragraph(_esc(v), cell_style)] for k, v in rows]
...
def generate_pdf_report(report, output_path, masked=False):
    if masked:
        report = apply_privacy_mask(report)

    signature_block = crypto_signing.sign_report(report)
    ...
    section_style = ParagraphStyle("SectionStyle", parent=styles["Heading2"], fontSize=12, spaceBefore=14, spaceAfter=6, textColor=colors.HexColor("#1b2a4a"))
    meta_style = ParagraphStyle("MetaStyle", parent=styles["Normal"], fontSize=9, textColor=colors.HexColor("#555555"))
    body_style = styles["Normal"]

    doc = SimpleDocTemplate(output_path, pagesize=A4, topMargin=20*mm, bottomMargin=20*mm, leftMargin=18*mm, rightMargin=18*mm)
    elements = []

    elements.append(Paragraph("Email Forensic Intelligence Report", title_style))
    elements.append(Paragraph("SIH 2026 \u00b7 PS ID 26106 \u00b7 Team Tech Titans (Prototype-stage report)", meta_style))
    ...
    score_table = Table([["Fraud Score", f"{ta['fraud_score']} / 100"], ["Verdict", ta["verdict"]]], colWidths=[100, 300])
    ...
    for reason in ta["reasons"]:
        elements.append(Paragraph(f"&bull; {_esc(reason)}", body_style))

    # ---- AI/ML Classification ----
    ml = ta.get("ml_classifier", {})
    elements.append(Paragraph("AI / ML Classification Engine", section_style))
    if ml.get("available"):
        _add_kv_table(elements, [
            ("Model", "Logistic Regression (TF-IDF, 5000 features)"),
            ("Trained On", "Enron Spam Dataset \u2014 33,665 real labeled emails"),
            ("Validated Accuracy", "98.87% (Precision 98.5%, Recall 99.3%, ROC-AUC 99.9%)"),
            ("This Email's Score", f"{(ml.get('probability') or 0)*100:.1f}% phishing/spam confidence \u2014 {str(ml.get('label') or 'unknown').upper()}"),
        ])
    else:
        _add_kv_table(elements, [("ML Classifier", "Unavailable for this analysis \u2014 scoring proceeded using the remaining signals only")])
    ...
    for evidence_text, ok in conf.get("evidence", []):
        elements.append(Paragraph(f"{'&#10003;' if ok else '&#10007;'} {_esc(evidence_text)}", body_style))
    ...
    for technique, flagged in report.get("threat_techniques", {}).items():
        elements.append(Paragraph(f"{'&#10003;' if flagged else '&#10007;'} {_esc(technique)}", body_style))
    ...
    elements.append(Paragraph(f"<i>Backend: {_esc(report.get('correlation_backend', 'Unknown'))}</i>", body_style))
    if corr:
        for m in corr:
            elements.append(Paragraph(
                "&bull; " + _esc(f"Shares {' + '.join(m['shared_on'])} with previous case: \"{m['subject']}\" (analyzed {m['timestamp'][:19]})"),
                body_style
            ))
    else:
        elements.append(Paragraph("No overlap with previously analyzed cases.", body_style))

    # ---- Fraud Campaign Intelligence ----
    cg = report.get("campaign_graph", {})
    elements.append(Paragraph("Fraud Campaign Intelligence", section_style))
    _add_kv_table(elements, [
        ("Campaign ID", cg.get("campaign_id", "\u2014")),
        ("Campaign Match Score", f"{cg.get('match_score', 0)}%"),
    ])

    # ---- Recommended Actions ----
    elements.append(Paragraph("Recommended Actions", section_style))
    for action in report.get("recommended_actions", []):
        elements.append(Paragraph(f"&bull; {_esc(action)}", body_style))

    # ---- Forensic Conclusion ----
    elements.append(Paragraph("Forensic Conclusion", section_style))
    elements.append(Paragraph(_esc(report.get("forensic_conclusion", "")), body_style))

    # ---- Privacy & Retention ----
    elements.append(Paragraph("Privacy &amp; Data Retention", section_style))
    elements.append(Paragraph(_esc(RETENTION_POLICY_TEXT), body_style))
    if masked:
        elements.append(Paragraph("<b>This report was generated in MASKED mode</b> — sender address and originating IP are partially redacted; underlying findings are unaffected.", body_style))

    # ---- Digital Signature / Chain of Custody ----
    elements.append(Paragraph("Digital Signature &amp; Chain of Custody", section_style))
    _add_kv_table(elements, [
        ("Algorithm", signature_block["algorithm"]),
        ("SHA-256 Hash", signature_block["sha256_hash"]),
        ("Signature (base64, truncated)", signature_block["signature_b64"][:80] + "..."),
        ("Signed At", signature_block["signed_at"]),
    ])
    elements.append(Paragraph(
        "This signature is generated by hashing the report's core findings (case ID, email summary, "
        "authentication results, origin trace, threat assessment, domain intelligence) and signing that "
        "hash with the platform's private key (RSA-2048). Anyone holding the platform's public key can "
        "independently verify this signature to confirm the findings have not been altered since analysis "
        "— this is the technical basis for chain-of-custody integrity.",
        meta_style
    ))

    elements.append(Spacer(1, 16))
    elements.append(Paragraph(
        "Prototype scope note: ML scoring uses a trained Logistic Regression model (Enron Spam Dataset); "
        "cross-case correlation uses Neo4j when configured, with an automatic local-store fallback. "
        "This is not a substitute for professional legal or security review.",
        meta_style
    ))

    doc.build(elements)
    return case_id, signature_block
