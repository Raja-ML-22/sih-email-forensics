...
# ---------- 2c. CAMPAIGN CORRELATION (real Neo4j graph, JSON fallback) ----------
# See neo4j_graph.py for the full implementation. Uses real Neo4j Aura when
# NEO4J_URI/NEO4J_USER/NEO4J_PASSWORD are configured; transparently falls
# back to a local JSON store otherwise so the app always keeps working.

import neo4j_graph

correlate_with_history = neo4j_graph.correlate_with_history
...
def score_email(parsed, auth_result, geo_result, domain_intel=None, correlation=None):
    ...
    # ---- ML text classifier (trained model, replaces pure keyword matching) ----
    try:
        import ml_classifier
        ml_prob, ml_label = ml_classifier.predict_phishing_probability(parsed["subject"], parsed["body"])
        if ml_prob >= 0.85:
            score += 35
        ...
        ml_prob, ml_label = None, None

    # Lightweight, explainable keyword signal kept alongside the ML model —
    # catches exact high-risk phrases even when overall ML confidence is moderate
    ...
    if correlation:
        score += 15
        matched_on = ", ".join(sorted(set(m for c in correlation for m in c["shared_on"])))
        reasons.append(
            f"Matches {len(correlation)} previously analyzed email(s) on {matched_on} — possible repeated campaign/infrastructure reuse"
        )
    ...
    return {
        ...
        "verdict": verdict,
        "reasons": reasons,
        "ml_classifier": {"probability": ml_prob, "label": ml_label},
    }
